...
export function isAPIServerError(e: unknown): e is APIServerError {
    //magic happens here
...
export class APIClient {
    ...
    post(path: string, data: unknown = {}, options: AxiosRequestConfig = {}) {
    ...
    ): Promise<AxiosResponse | APIServerError> {
    ...
    wait(s = 0) {
        return new Promise((resolve, reject) => {
            ...
            if (s > 0) {
                new Promise(r => setTimeout(r, s * 1000)).then(() => {
                    cancel = true;
                });
            }
